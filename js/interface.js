...
function updateGCodeFiles() {
	...
	if (gcodeUpdateIndex == -1) {
		...
		var row = gCodeTable.row(gcodeUpdateIndex);
...
function clearGCodeFiles() {
	if(!gCodeTable) {
		gCodeTable = $("#table_gcode_files").removeClass('hidden').DataTable(gCodeTableOptions);
	}
	gCodeTable.rows().remove().draw();
}
...
function parseSize(size) {
	...
	switch(parts[1]) {
		case 'GiB':
			return parts[0] * 1073741824;
		case 'MiB':
			return parts[0] * 1048576;
		case 'KiB':
			return parts[0] * 1024;
		case 'GB':
			return parts[0] * 1000000000;
		case 'MB':
...
function setGCodeFileItem(row, size, height, firstLayerHeight, layerHeight, filamentUsage, generatedBy) {
	$(row.node()).data("file", $(row.node()).data("item")).removeData("item");

	var d = row.data();

	d[0] = '<button class="btn btn-success btn-print-file btn-sm" title="' + T("Print this file (M32)") + '"><span class="glyphicon glyphicon-print"></span></button>';
	d[1] = '<button class="btn btn-danger btn-delete-file btn-sm" title="' + T("Delete this file") + '"><span class="glyphicon glyphicon-trash"></span></button>';

	var linkCell = $('<a/>').addClass('gcode-file').attr('href', '#').html(d[2]);
	linkCell.find("span").removeClass("glyphicon-asterisk").addClass("glyphicon-file");
	d[2] = linkCell[0].outerHTML;
	...
	if (layerHeight > 0) {
		if (firstLayerHeight == undefined) {
			...
			d[5] = firstLayerHeight + " / " + layerHeight + " mm";
		}
	} else {
		d[5] = "n/a";
	}
...
function setGCodeDirectoryItem(row) {
	...
	var linkCell = $('<a/>').addClass('gcode-directory').attr('href', '#').html(d[2]);
